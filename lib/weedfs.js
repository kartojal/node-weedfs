...
function WeedFSClient(opts) {
    ...
    if (this.clientOpts.port) {
        this.baseURL = "https://" + this.clientOpts.server + ":" + this.clientOpts.port + "/";
    } else {
        this.baseURL = "https://" + this.clientOpts.server + "/";
    }
}
        ...
        return self._assign(opts).then(function(finfo) {
            ...
            for (var i = 0; i < opts.count; i++) {
                proms.push(new Promise(function(resolve, reject) {
                    ...
                    var url = "https://" + (self.usePublicUrl ? finfo.publicUrl : finfo.url) + "/" + finfo.fid + (opts.count == 1 ? "" : "_" + i);
